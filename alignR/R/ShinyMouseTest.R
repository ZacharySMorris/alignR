...
server <- function(input, output, session) {
  ...
  output$brush_info_3D <- renderPrint(input$rgl_3D_brush$region[1:2]) #This gets the "x" and "y" index values from the brush click....but why is it only two dimensions?
  ...
  PlotTemp <- reactive({
    ...
    # points3d(dat[, 1:3], size = 10, color = "red", add = TRUE)
